...
class SaverWorkspaceBackHandlerTest : ComposeTest() {
    ...
    private fun setPage(vm: SaverWorkspaceViewModel, isModal: Boolean): () -> Unit {
                ...
                PaneLayerHost(modifier = Modifier.fillMaxSize(), paneFocused = true) {
                    PaneLayer(
                        ...
                        SaverWorkspacePage(
                            ...
                            stateSource = flowOf(SaverWorkspaceViewModel.State(isModal = isModal)),
                            vm = vm,
                        )
                    }
                }
    ...
    fun `modal saver closes on back`() {
        val vm = mockVm()
        val pressBack = setPage(vm, isModal = true)
    ...
    fun `share intent saver ignores back`() {
        val vm = mockVm()
        val pressBack = setPage(vm, isModal = false)
