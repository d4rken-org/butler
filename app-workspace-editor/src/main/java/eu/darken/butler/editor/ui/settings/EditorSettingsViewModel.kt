...
constructor(
    ...
    ) { showLineNumbers, wordWrap, showMemoryStats ->
        State(
    ...
    fun updateShowMemoryStats(enabled: Boolean) = launch {
        log(tag) { "updateShowMemoryStats($enabled)" }
        editorSettings.showMemoryStats.value(enabled)
    }
