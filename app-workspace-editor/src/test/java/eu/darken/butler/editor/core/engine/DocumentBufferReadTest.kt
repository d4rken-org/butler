...
/**
 * Tests for DocumentBuffer read operations including line counting, content display,
 * chunk boundaries, position/offset conversion, and edge cases.
 */
class DocumentBufferReadTest : DocumentBufferTestBase() {
    ...
    fun `small chunks - getTextForRange crossing multiple chunks`() = runTest {
        ...
        val content = lines.joinToString("\n")
        val buffer = createBuffer(content, blockSize = 500)
    ...
    fun `small chunks - empty chunk at end`() = runTest {
        ...
        val buffer = createBuffer(content, blockSize = 50)
    ...
    fun `getText at exact chunk boundary returns correct content`() = runTest {
        ...
        val content = chunk1 + chunk2
        val buffer = createBuffer(content, blockSize = 4)
    ...
    fun `last chunk with adjusted START boundary is readable`() = runTest {
        ...
        val buffer = createBuffer(fullContent, blockSize = 100)
