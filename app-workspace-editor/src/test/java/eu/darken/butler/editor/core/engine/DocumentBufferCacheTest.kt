...
/**
 * Tests for DocumentBuffer under small-block, high-churn scenarios (ported from the
 * ChunkedTextBuffer eviction/cache-pressure tests). The new engine has no chunk cache,
 * so these tests assert observable behavior only: same content and edit sequences,
 * verified via getText/getTextForRange/totalLength/totalLines.
 */
class DocumentBufferCacheTest : DocumentBufferTestBase() {
    ...
    fun `searching across evicted chunks works correctly`() = runTest {
        ...
        val content = chunks.joinToString("")
        val buffer = createBuffer(content, blockSize = 11)
        ...
        val results = buffer.search("Line", options = SearchOptions(caseSensitive = true))
    ...
    fun `getTextForRange across evicted chunks returns correct content`() = runTest {
        ...
        val buffer = createBuffer(content, blockSize = 20)  // ~10 chunks
    ...
    fun `line count remains accurate with chunk eviction`() = runTest {
        ...
        val buffer = createBuffer(lines, blockSize = 10)
    ...
    // `saving file with evicted dirty chunks works correctly` — requires a successful
    // saveFile() on a modified buffer.
}
