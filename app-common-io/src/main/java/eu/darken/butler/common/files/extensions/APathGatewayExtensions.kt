...
suspend fun <P : APath<P>, PL : APathLookup<P>> P.lookup(gateway: APathGateway<P, PL>): PL {
    return gateway.lookup(this)
}

suspend fun <P : APath<P>, PL : APathLookup<P>> P.lookupFiles(gateway: APathGateway<P, PL>): Collection<PL> {
    return gateway.lookupFiles(this)
}

suspend fun <P : APath<P>, PL : APathLookup<P>> P.lookupFilesOrNull(gateway: APathGateway<P, PL>): Collection<PL>? {
    return if (exists(gateway)) gateway.lookupFiles(this) else null
}

suspend fun <T : APath<T>> T.listFiles(gateway: APathGateway<T, out APathLookup<T>>): Collection<T> {
    return gateway.listFiles(this)
}

suspend fun <T : APath<T>> T.canRead(gateway: APathGateway<T, out APathLookup<T>>): Boolean {
    return gateway.canRead(this)
}

suspend fun <T : APath<T>> T.canWrite(gateway: APathGateway<T, out APathLookup<T>>): Boolean {
    return gateway.canWrite(this)
}

suspend fun <T : APath<T>> T.isFile(gateway: APathGateway<T, out APathLookup<T>>): Boolean {
    return gateway.lookup(this).fileType == FileType.FILE
}

suspend fun <T : APath<T>> T.isDirectory(gateway: APathGateway<T, out APathLookup<T>>): Boolean {
    return gateway.lookup(this).fileType == FileType.DIRECTORY
}

suspend fun <T : APath<T>> T.getFileSystemInfo(gateway: APathGateway<T, out APathLookup<T>>): FileSystem {
