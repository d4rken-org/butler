...
import eu.darken.butler.workspace.ui.scroll.TopBarScrollState.Companion.Saver
import kotlinx.coroutines.CancellationException
import kotlin.math.abs

/**
 * A scroll behavior for top bars that hides completely when scrolling down and shows when scrolling up.
 * Unlike [TopToolbarScrollBehavior] which collapses to a smaller size, this completely hides the bar.
 */
@ExperimentalMaterial3Api
@Stable
class TopBarScrollBehavior(
    val state: TopBarScrollState = TopBarScrollState(),
...
/**
 * State for managing the scroll behavior of a top bar that hides completely.
 */
@ExperimentalMaterial3Api
@Stable
class TopBarScrollState(
    ...
    /**
     * The top bar's height in pixels.
     */
    var height: Float by mutableFloatStateOf(initialContentHeight)

    /**
     * The top bar's current height offset in pixels. This value is always <= 0, and represents
     * how much the bar is currently offset upwards to hide it.
     * 0 represents a fully shown top bar, while negative values represent a hidden bar.
     */
    var heightOffset: Float by mutableFloatStateOf(initialHeightOffset)

    /**
     * The limit for the top bar's height offset.
     */
    var heightOffsetLimit by mutableFloatStateOf(initialHeightOffsetLimit)

    /**
     * Whether the top bar can scroll (i.e., is not completely collapsed/expanded and has room to scroll).
     */
    val canScroll: Boolean
        get() = heightOffset > heightOffsetLimit && heightOffset < 0

    /**
     * A value that represents how much the top bar is collapsed.
    ...
    companion object {
        /**
         * The default [Saver] implementation for [TopBarScrollState].
         */
...
/**
 * Create and remember a [TopBarScrollBehavior] with the default parameters.
 */
@ExperimentalMaterial3Api
@Composable
fun rememberTopBarScrollBehavior(
    state: TopBarScrollState = rememberTopBarScrollState(),
    snapAnimationSpec: AnimationSpec<Float> = spring(stiffness = Spring.StiffnessMediumLow),
    flingAnimationSpec: FloatDecayAnimationSpec? = null,
): TopBarScrollBehavior = remember(state, snapAnimationSpec, flingAnimationSpec) {
    TopBarScrollBehavior(state, snapAnimationSpec, flingAnimationSpec)
}

/**
 * Create and remember a [TopBarScrollState].
 */
@ExperimentalMaterial3Api
@Composable
fun rememberTopBarScrollState(): TopBarScrollState {
...
/**
 * Returns the height of the top bar in dp for the given scroll state.
 */
@ExperimentalMaterial3Api
@Composable
fun TopBarScrollState.getHeightDp(): Dp {
    val density = LocalDensity.current
    return with(density) { height.toDp() }
}

/**
 * Sets the height of the top bar for the scroll state.
 */
@ExperimentalMaterial3Api
@Composable
fun TopBarScrollState.setHeight(heightDp: Dp) {
    val density = LocalDensity.current
