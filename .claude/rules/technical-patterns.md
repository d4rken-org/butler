...
- Pinning names: `@Keep` on our own classes resolved by name, module `consumer-rules.pro` for generated code that cannot carry an annotation (AIDL `Stub`/`Proxy`), `-keepnames` for hierarchies whose names are displayed. `-keepclassmembers` keeps members but does NOT pin the class name.
