...
class ReviewSettingsTest : BaseTest() {
    ...
    fun `the review timestamps round-trip through the DataStore`() = runTest {
        ...
        withContext(Dispatchers.IO) {
            ...
            // Pins the loud-failure behaviour: these values are created with
            // onErrorFallbackToDefault = false, so unreadable data surfaces instead of silently
            // reading as "never reviewed" (which would re-ask a user who already left a review).
