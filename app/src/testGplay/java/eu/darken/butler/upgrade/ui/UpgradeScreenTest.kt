...
class UpgradeScreenTest : ComposeTest() {
    ...
    fun `a pending payment shows the card and locks the offers for acquisition`() {
        ...
        composeTestRule.onAllNodesWithTag(UpgradeScreenTags.PENDING).assertCountEquals(1)
        composeTestRule.onAllNodesWithText(context.getString(R.string.upgrade_screen_pending_card_body))
            .assertCountEquals(1)
        ...
        composeTestRule.onNodeWithTag(UpgradeScreenTags.RESTORE).assertIsEnabled()
    }
    ...
    fun `the pending dialog is informational only`() {
        var dismissals = 0
        composeTestRule.setUpgradeContent { PurchasePendingDialog(onDismiss = { dismissals++ }) }
        ...
        composeTestRule.onAllNodesWithText(context.getString(R.string.upgrade_screen_contact_support_action))
            .assertCountEquals(0)
        composeTestRule.onAllNodesWithText(context.getString(R.string.upgrade_screen_restore_multiaccount_hint))
            .assertCountEquals(0)

        composeTestRule.onNodeWithText(context.getString(CommonR.string.general_dismiss_action)).performClick()
        composeTestRule.runOnIdle { dismissals shouldBe 1 }
    }
