 ...
 * the live info flow publishes too), so a paused tab describes itself exactly like the location
 * resuming it actually opens — including the second line targets such as Trash carry. Pure and
 * synchronous — used for both the paused stand-in and the live workspace's
 ...
 * paused derivation and the live info flow so both describe a location the same way.
 */
