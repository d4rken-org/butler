...
class FlowCmdShell(
    ...
    data class Session(
        ...
        /**
         * Whether [execute] would still accept a command.
         *
         * The process exit code is published by a separate exit-monitor coroutine, so it lags behind
         * the stream EOF that already made [execute] reject everything: [isAlive] alone can say
         * "alive" for a session that throws on its very next command. Only ever a snapshot - EOF can
         * always land right after the answer is given - but it must not be stale by construction,
    ...
    companion object {
        ...
        // env var assignments). Heuristic, not a real shell parser — it may match a quoted
        // `exec` and miss exotic forms. Used to recognise commands where the shell
        // intentionally replaces itself with another process — no `echo idEnd` will be
        // emitted in that case, so execute() must surface completion as ExitCode(-1) instead
        // of throwing. A missing marker from an *external* shell death still throws.
