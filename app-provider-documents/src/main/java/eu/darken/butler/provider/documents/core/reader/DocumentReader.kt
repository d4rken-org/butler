    ...
    private suspend fun openForRead(path: APath<*>): ParcelFileDescriptor {
        ...
        if (path is LocalPath) {
            try {
                ...
                log(TAG, WARN) { "Direct local PFD failed, trying gateway-backed reader: ${e.message}" }
            }
        }
        ...
        return try {
            ...
            createSeekableFromFileHandle(fileHandle)
        } catch (e: Exception) {
            log(TAG, WARN) { "Seekable PFD failed, falling back to pipe: ${e.message}" }
            createPipeFromInputStream(gatewaySwitch.openInputStream(path))
        }
    }
