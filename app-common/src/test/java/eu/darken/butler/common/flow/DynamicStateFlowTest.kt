...
import eu.darken.butler.common.collections.mutate
...
class DynamicStateFlowTest : BaseTest() {
    ...
    fun `concurrent blocking updates survive a reactive collector`() {
        ...
        try {
            ...
            // Mirrors a reactive consumer: a collector that answers every state with an update of
            // its own, keeping the producer busy between the other callers' updates. The echo
            // updates are value-neutral so the final count stays exact, and capped so the echo
            // terminates.
