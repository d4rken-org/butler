...
# Each locale renders 23 images (7 phone + 8 seven-inch + 8 ten-inch).
#
# Usage:
...
# 7 phone + 8 seven-inch + 8 ten-inch. Not a clean multiple: the phone set drops the explorer home
# shot. Keep in sync with PlayStoreScreenshots.kt and the per-form-factor counts in
# copy_screenshots.sh.
RENDERS_PER_LOCALE=23

# One locale per batch. 23 renders already hold a few hundred MB of leaked pixels in the
...
emit_previews() {
    ...
    for (( i = start; i < start + count; i++ )); do
        echo "@Preview(locale = \"${QUALIFIERS[$i]}\", name = \"${DIRS[$i]}\", device = $spec)"
    done
}
