...
a caveat. Recorded output lands in `<permission>/declaration.mp4`, next to the declaration it
belongs to. Those files are git-ignored: they stay on this machine and are never committed.
...
The videos are regenerated by script with no manual screen-recording. Each
...
captions, and an end card. **The scripts are committed; the recorded output is not.** Each recorder writes into its own
folder (`<permission>/declaration.mp4`, plus `raw.mp4` and the caption/card text), and
`.gitignore` keeps all of it out of the repository. The videos therefore sit with the
declaration they document without ever being checked in. Override with `OUTDIR=<dir>`.
...
```bash
# The target must have Butler installed. Pass the adb serial as the first argument.

# All-files-access demo
./manage-external-storage/record.sh <serial>
./postprocess.sh ./manage-external-storage

# App-manager (query-all-packages) demo
./query-all-packages/record.sh <serial>
./postprocess.sh ./query-all-packages
...
# not; reset it first by reinstalling Butler:
#   adb -s <serial> uninstall eu.darken.butler
#   adb -s <serial> install -g app/build/outputs/apk/foss/debug/app-foss-debug.apk
./request-install-packages/record.sh <serial>
./postprocess.sh ./request-install-packages

# Foreground-service demo — REAL DEVICE ONLY, see the note above. SIZE must match
# the device's aspect ratio or the frame is squeezed (720x1480 for a 1080x2220
# panel); postprocess.sh takes its frame size from the recording.
SIZE=720x1480 ./foreground-service-data-sync/record.sh <serial>
./postprocess.sh ./foreground-service-data-sync

# NOREC=1 ./<permission>/record.sh ...   # validate the tap chain without recording
```

Each output is portrait H.264, under the 2-minute limit, and uses only
synthetic/seeded data, so nothing personal is filmed. The frame size follows the
capture rather than being pinned: 720×1606 from an emulator, 720×1480 from a
1080×2220 phone.
